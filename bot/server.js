...
// ════════════════════════════════════════════════════════════
// Dual-Mode: AUTOPILOT (local Ollama) vs BRAIN-MODE (external ARES)
// ════════════════════════════════════════════════════════════

let brainMode = 'autopilot'; // 'autopilot' | 'brain'
let brainEndpoint = null;    // URL of external ARES brain
let brainApiKey = null;
let brainLastActivity = 0;
const BRAIN_TIMEOUT_MS = 60000; // Revert to autopilot if no activity
let eventSubscribers = [];   // SSE connections

function pushEvent(event) {
  const payload = JSON.stringify({ ...event, time: Date.now() });
  eventSubscribers = eventSubscribers.filter(sub => {
    try {
      sub.write(`data: ${payload}\n\n`);
      return true;
    } catch {
      return false; // drop dead connections
    }
  });
}

function isBrainMode() {
  if (brainMode !== 'brain' || !brainEndpoint) return false;
  if (Date.now() - brainLastActivity > BRAIN_TIMEOUT_MS) {
    brainMode = 'autopilot';
    brainEndpoint = null;
    pushEvent({ type: 'mode_change', mode: 'autopilot' });
    log('Brain connection timed out. Switched to AUTOPILOT.');
    return false;
  }
  return true;
}

function setBrainMode(endpoint, apiKey) {
  brainMode = 'brain';
  brainEndpoint = endpoint;
  brainApiKey = apiKey;
  brainLastActivity = Date.now();
  pushEvent({ type: 'mode_change', mode: 'brain', endpoint });
  log(`Brain connected: ${endpoint}`);
}
...
function disconnectBrain() {
  if (brainMode === 'brain') {
    brainMode = 'autopilot';
    pushEvent({ type: 'mode_change', mode: 'autopilot' });
    log('Brain disconnected. Switched to AUTOPILOT.');
  }
  brainEndpoint = null;
  brainApiKey = null;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    bot.once('spawn', () => {
      ...
      bot.on('chat', (username, message) => {
        ...
        // Push event to brain if active
        if (isBrainMode()) pushEvent({ type: 'chat', from: username, message: message });
      });
      ...
      bot.on('whisper', (username, message) => {
        ...
        if (commandQueue.length > MAX_QUEUE) commandQueue.shift();
        // Push event to brain
        if (isBrainMode()) pushEvent({ type: 'whisper', from: username, message: message });
      });
      ...
      bot.on('death', () => {
        combatStats.deaths++;
        pushEvent({ type: 'death', position: posObj(), inventory: bot.inventory.items().map(i => ({ name: i.name, count: i.count })) });
        lastDeath = {
          time: Date.now(),
          position: posObj(),
...
const httpServer = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (!actionMatch) {
        // ── Brain Control ──────────────────────────
        if (path === '/brain/connect') {
          const { endpoint, api_key } = body;
          if (!endpoint) return respond(res, 400, { ok: false, error: 'Missing "endpoint" body field' });
          setBrainMode(endpoint, api_key || null);
          return respond(res, 200, { ok: true, mode: 'brain', endpoint });
        }
        if (path === '/brain/disconnect') {
          disconnectBrain();
          return respond(res, 200, { ok: true, mode: 'autopilot' });
        }
        if (path === '/brain/thought') {
          if (!body.thought) return respond(res, 400, { ok: false, error: 'Missing "thought" body field' });
          brainLastActivity = Date.now();
          // Validate and execute thought as an action if structured
          const thoughtRe = /^ACTION:\s*(\w+).*PARAMS:\s*(\{[\s\S]*\})/i;
          const m = body.thought.match(thoughtRe);
          if (m) {
            const name = m[1].toLowerCase();
            let p;
            try { p = JSON.parse(m[2]); } catch { p = {}; }
            const fn = ACTIONS[name];
            if (fn) {
              fn(p).then(r => {
                pushEvent({ type: 'brain_action', action: name, result: r.result });
              }).catch(e => {
                pushEvent({ type: 'brain_action_error', action: name, error: e.message });
              });
              return respond(res, 200, { ok: true, accepted: name, state: briefState() });
            }
          }
          pushEvent({ type: 'brain_thought', thought: body.thought });
          return respond(res, 200, { ok: true, mode: brainMode });
        }
        // ── SSE ────────────────────────────────────
        if (path === '/events') {
          res.writeHead(200, {
          ...
          const hb = { type: 'heartbeat', mode: brainMode };
